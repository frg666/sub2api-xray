...
**Sub2API にユーザー専用リソースと Xray プロキシを追加**
...
本リポジトリは独立して保守されており、上流プロジェクトの公式配布物ではありません。上流の変更は検証後に取り込み、Xray 固有のバージョンには `v<upstream>-xray<revision>` 形式を使用します。更新確認先も本リポジトリに分離されています。

## 上流版との主な違い

| 領域 | Sub2API Xray の追加機能 |
|------|-------------------------|
| ユーザーリソース | 一般ユーザーが自分専用のグループ、アカウント、プロキシ、割り当てサブスクリプション、交換コードを管理できます。 |
| グループとアカウント | ユーザー向けフォームを管理者フローと揃え、ルーティング、クォータ、倍率、RPM、インポート/エクスポート、テスト、一括操作に対応します。 |
| プロキシランタイム | 標準 HTTP/SOCKS に加え、Xray 経由の `vmess`、`vless`、`trojan`、Shadowsocks、SOCKS、HTTP ノードを利用できます。Base64 および Clash サブスクリプションも同期できます。 |
| サブスクリプション配布 | ユーザーによる直接割り当てと交換コード配布に対応し、複数回利用コードと交換履歴詳細も提供します。 |
| ヘルス表示 | 購読者はアカウントプールの状態を確認し、利用できないサブスクリプションを解除できます。 |
| 利用診断 | 自分の利用履歴、自分のアカウントの利用履歴、秘匿化された上流エラーを管理者画面と同等の項目で確認できます。 |
| 所有権と安全性 | 非公開リソースは所有者単位で分離されます。公開プロキシは安全なメタデータのみを表示し、他ユーザーのリソースにはアクセスできません。 |

ユーザーリソース画面は `enable_user_resources` で制御され、既存環境のアップグレード時はデフォルトで無効です。

## 現在のリリース

現在の統合テスト版は、上流 `v0.1.166` をベースにした `v0.1.166-xray3-beta1` です。最新の正式リリースは引き続き `v0.1.165-xray3` です。詳細は [GitHub Release](https://github.com/SMNNagarajan/sub2api-xray/releases/tag/v0.1.165-xray3) を参照してください。

## 重要なお知らせ

- **利用規約のリスク**：上流アカウントの利用がプロバイダー規約に抵触する場合があります。導入前に関連規約を確認してください。
- **法令遵守**：適用される法令で認められた地域および用途に限って運用してください。
- **データ保護**：アカウント認証情報、プロキシサブスクリプション、API キー、ログは機密情報です。強力なシークレット、HTTPS、バックアップ、管理者権限の制限を使用してください。
- **商用利用の非許諾**：本リポジトリは第三者によるプロジェクト名義での商業運営を許諾しません。運営者は自身のサービスとユーザーに独立して責任を負います。

## 概要

Sub2API Xray は AI 製品サブスクリプションのクォータを配布・管理する API ゲートウェイです。プラットフォームが発行する API キーを通じて上流サービスを利用し、認証、課金、負荷分散、スケジューリング、リクエスト転送を処理します。
